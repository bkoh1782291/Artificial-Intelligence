...
# displaying all the possible game states
def allstates(new_b):
    
    file = open("gameStates.txt", "a")
    file.write("")
    file.close()

    next_best_move = findBestMove(new_b)
    ...
    if O_count > X_count:
        new_board[next_best_move[0][0]][next_best_move[0][1]] = 'x'
    elif X_count > O_count:
        new_board[next_best_move[0][0]][next_best_move[0][1]] = 'o'
    else:
        new_board[next_best_move[0][0]][next_best_move[0][1]] = 'x'
...
# function to split string into 2d matrix
def stringsplit(board):
...
# update value of the board after best move
def updateboard(b):
    # check amount of x's and o's on the board 
    O_count = 0
    X_count = 0
...
# converts 2d matrix to string
def list_to_string(new_b):
    # convert list to string
    new_str = ""
...
if __name__ == "__main__":
    ...
    # capture string input and split string into 2D Matrix
    board = sys.argv[1]
    b = stringsplit(board)

    # call best move function
    bestMove = findBestMove(b)
    print bestMove[0]

    # update board value and return string from 2d matrix
    new_board = updateboard(b)
    new_str = list_to_string(new_board)
    print new_str
